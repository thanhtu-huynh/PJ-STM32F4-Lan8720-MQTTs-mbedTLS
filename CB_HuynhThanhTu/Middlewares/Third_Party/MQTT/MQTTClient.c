...
long tick_tu_test = 0;
int MQTTYieldNoTimer(MQTTClient* c, int timeout_ms)
{
    int rc = MQTT_SUCCESS;
    //static long start = 0;   // Lấy thời điểm bắt đầu (ms)

    if ((HAL_GetTick() - tick_tu_test) > timeout_ms)  // Lặp cho đến khi hết timeout
    {
    	tick_tu_test = HAL_GetTick();
        if (cycle(c, NULL) < 0)      // Xử lý một chu kỳ MQTT
        {
            rc = FAILURE;
